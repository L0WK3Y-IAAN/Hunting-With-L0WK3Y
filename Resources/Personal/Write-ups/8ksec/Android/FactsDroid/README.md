...
f503f didn’t lead anywhere interesting, but `f502g` revealed that the application is performing root checks, I make sure to note this down for late.
...
![Enable “Invisible Proxying” in Burp](L0WK3Y%20-%208ksec%20FactsDroid%20Walkthrough/8ec8615e-b014-4982-8855-00c3e7cb854a.png)
...
- **DNAT:**
    ...
    *Forces the app’s traffic, which is going straight to `uselessfacts.jsph.pl:443` (for `https://uselessfacts.jsph.pl/api/v2/facts/random`), to instead go to your Burp Suite listener without the app realizing anything changed.*
    
- **Invisible proxy:**
    
    *Tells Burp to accept these “normal” direct HTTP(S) connections (not proxy-style requests) and still forward them correctly, as if it really were `uselessfacts.jsph.pl`.*
...
![Enable the Intercept option “Response to this request”](L0WK3Y%20-%208ksec%20FactsDroid%20Walkthrough/25d8bf86-a625-4e3b-b553-4f1bcc0fb0af.png)
...
2. The engine’s own setting about “may insecurely connect to all domains”.
...
- **`false`** → block non‑HTTPS connections (except loopback).
